...
- first, the programme is compiled according to instructions, which will produce an .exe file; by default, the programme assumes an initial random state.
...
- the steps above are repeated recompiling the programme such to start from an ordered state.
