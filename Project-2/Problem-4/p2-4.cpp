...
int main() { 
    ...
    arma::mat A {{1, 0, 0, 0.5, 0, 1}, {0, -0.4, -0.7, 0, 2, 1.2}, {0, -0.7, 1, 0, -0.3, 0.1}, {0.5, 0, 0, 1, -0.5, 0},  {0, -0.7, 1, 0, 0, 1,}, {0.5, 0, 0, 1, -0.7, -0.9}};
    ...
    while  ( max > eps) {
        ...
        // find indices of new max off diag element
        max = abs_max_offdiag_for_symmetric(A, pos[0], pos[1]);
        max=abs(max);
    }
    ...
    // print
    std::cout << std::endl <<  "Eigenvalues from the Jacobi rotation method:" << std::endl;
    std::cout  << std::setprecision(4)  << eigval << std::endl;

    std::cout << std::endl <<  "Eigenvectors from the Jacobi rotation method:" << std::endl;
